...
public sealed class FavoriteService : IFavoriteService
{
    ...
    /// <inheritdoc/>
    public Task<(IReadOnlyList<VideoFavoriteFolder> Folders, IReadOnlyList<string> ContainerIds)> GetPlayingVideoFavoriteFoldersAsync(string aid, CancellationToken cancellationToken = default)
        => _myClient.GetPlayingVideoFavoriteFoldersAsync(aid, cancellationToken);
    ...
    /// <inheritdoc/>
    public Task<(IReadOnlyList<VideoInformation> Videos, int TotalCount, int? NextPageNumber)> GetUgcSeasonVideosAsync(VideoFavoriteFolder folder, int pageNumber = 0, CancellationToken cancellationToken = default)
    {
        if (pageNumber < 0)
        {
            throw new KernelException("页码不能小于0");
        ...
        pageNumber++;
        return _myClient.GetUgcSeasonVideosAsync(folder, pageNumber, cancellationToken);
    }
