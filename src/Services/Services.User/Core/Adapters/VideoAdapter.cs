...
internal static class VideoAdapter
{
    ...
    public static VideoInformation ToVideoInformation(this UgcSeasonArchive archive)
    {
        var cover = archive.pic.ToVideoCover();
        var communityInfo = new VideoCommunityInformation(archive.aid.ToString(), archive.stat.view);
        var progress = archive.playback_position;
        var identifier = new MediaIdentifier(archive.aid.ToString(), archive.title, cover);
        ...
        var info = new VideoInformation(
            ...
            publishTime: publishTime,
            communityInformation: communityInfo);
        info.AddExtensionIfNotNull(VideoExtensionDataId.Progress, progress);
        info.AddExtensionIfNotNull(VideoExtensionDataId.MediaType, MediaType.Video);
        return info;
    }
