...
internal sealed class MyClient
{
    ...
    {
        var responseObj = await GetAsync(BiliApis.Account.MyInfo, JsonContext.Default.BiliDataResponseMyInfo, cancellationToken: cancellationToken).ConfigureAwait(false);
    ...
    {
        var responseObj = await GetAsync(BiliApis.Account.Mine, JsonContext.Default.BiliDataResponseMine, cancellationToken: cancellationToken).ConfigureAwait(false);
    ...
    {
        var responseObj = await GetAsync(BiliApis.Account.MyFollowingTags, JsonContext.Default.BiliDataResponseListRelatedTag, cancellationToken: cancellationToken).ConfigureAwait(false);
    ...
    public async Task<IReadOnlyList<UserCard>> GetMyFollowUserGroupDetailAsync(string groupId, int page = 1, CancellationToken cancellationToken = default)
    {
        ...
        var responseObj = await GetAsync(BiliApis.Account.MyFollowingTagDetail, JsonContext.Default.BiliDataResponseListRelatedUser, parameters, cancellationToken).ConfigureAwait(false);
    ...
    public async Task<(IReadOnlyList<UserCard> Users, int Count)> GetMyFansAsync(int page = 1, CancellationToken cancellationToken = default)
    {
        ...
        var responseObj = await GetAsync(BiliApis.Account.Fans, JsonContext.Default.BiliDataResponseRelatedUserResponse, parameters, cancellationToken).ConfigureAwait(false);
    ...
    public async Task<UserRelationStatus> GetRelationshipAsync(string userId, CancellationToken cancellationToken)
    {
        ...
        var responseObj = await GetAsync(BiliApis.Account.Relation, JsonContext.Default.BiliDataResponseUserRelationResponse, parameters, cancellationToken).ConfigureAwait(false);
    ...
    {
        var responseObj = await GetAsync(BiliApis.Account.MessageUnread, JsonContext.Default.BiliDataResponseUnreadMessage, cancellationToken: cancellationToken).ConfigureAwait(false);
    ...
    public async Task<(IReadOnlyList<ChatMessage> Messages, long MaxNumber, bool HasMore)> GetChatMessagesAsync(UserProfile user, int messageCount, CancellationToken cancellationToken)
    {
        ...
        var responseObj = await BiliHttpClient.ParseAsync(response, JsonContext.Default.BiliDataResponseChatMessageResponse).ConfigureAwait(false);
    ...
    public async Task<ChatMessage> SendChatMessageAsync(string content, UserProfile user, CancellationToken cancellationToken)
    {
        ...
        {
            { "msg[msg_type]", "1" },
            { "msg[content]", JsonSerializer.Serialize(new ContentString { Content = content }, JsonContext.Default.ContentString) },
        ...
        var response = await _httpClient.SendAsync(request, cancellationToken).ConfigureAwait(false);
        var responseObj = await BiliHttpClient.ParseAsync(response, JsonContext.Default.BiliDataResponseSendMessageResponse).ConfigureAwait(false);
    ...
    public async Task<(IReadOnlyList<ChatSession> Sessions, long Offset, bool HasNext)> GetChatSessionsAsync(long? offset, CancellationToken cancellationToken)
    {
        ...
        var response = await _httpClient.SendAsync(request, cancellationToken).ConfigureAwait(false);
        var responseObj = await BiliHttpClient.ParseAsync(response, JsonContext.Default.BiliDataResponseChatSessionListResponse).ConfigureAwait(false);
    ...
    public async Task<IReadOnlyList<UserProfile>> BatchGetUsersAsync(IEnumerable<string> userIds, CancellationToken cancellationToken)
    {
        ...
        var userResponseObj = await BiliHttpClient.ParseAsync(userResponse, JsonContext.Default.BiliDataResponseListBiliChatUser).ConfigureAwait(false);
    ...
    public async Task<(IReadOnlyList<NotifyMessage> Messages, long OffsetId, long OffsetTime, bool HasMore)> GetNotifyMessagesAsync(NotifyMessageType type, long offsetId, long offsetTime, CancellationToken cancellationToken)
    {
        ...
        {
            var responseObj = await BiliHttpClient.ParseAsync(response, JsonContext.Default.BiliDataResponseLikeMessageResponse).ConfigureAwait(false);
            cursor = responseObj.Data.Total.Cursor;
            messages = responseObj.Data.ToNotifyMessages();
        }
        else if (type == NotifyMessageType.Reply)
        {
            var responseObj = await BiliHttpClient.ParseAsync(response, JsonContext.Default.BiliDataResponseReplyMessageResponse).ConfigureAwait(false);
            cursor = responseObj.Data.Cursor;
            messages = responseObj.Data.ToNotifyMessages();
        }
        else if (type == NotifyMessageType.At)
        {
            var responseObj = await BiliHttpClient.ParseAsync(response, JsonContext.Default.BiliDataResponseAtMessageResponse).ConfigureAwait(false);
            cursor = responseObj.Data.Cursor;
            messages = responseObj.Data.ToNotifyMessages();
        }
    ...
    public async Task<(IReadOnlyList<VideoFavoriteFolderGroup>, VideoFavoriteFolderDetail)> GetVideoFavoriteGroupsAsync(CancellationToken cancellationToken)
    {
        ...
        var galleryResponse = await GetAsync(BiliApis.Account.VideoFavoriteGallery, JsonContext.Default.BiliDataResponseVideoFavoriteGalleryResponse, parameters, cancellationToken).ConfigureAwait(false);
        ...
        var defaultFolderResponse = await GetAsync(BiliApis.Account.FavoriteList, JsonContext.Default.BiliDataResponseFavoriteDetailListResponse, parameters, cancellationToken).ConfigureAwait(false);
        ...
        var folderResponseObj = await BiliHttpClient.ParseAsync(await folderResponse, JsonContext.Default.BiliDataResponseFavoriteDetailListResponse).ConfigureAwait(false);
    ...
    public async Task<(IReadOnlyList<ArticleInformation>, int)> GetFavoritesArticlesAsync(int pageNumber, CancellationToken cancellationToken)
    {
        ...
        var responseObj = await GetAsync(BiliApis.Account.ArticleFavorite, JsonContext.Default.BiliDataResponseArticleFavoriteListResponse, parameters, cancellationToken).ConfigureAwait(false);
    ...
    public async Task<(IReadOnlyList<SeasonInformation>, int)> GetFavoritePgcListAsync(string url, int pageNumber, PgcFavoriteStatus status, CancellationToken cancellationToken)
    {
        ...
        var responseObj = await GetAsync(url, JsonContext.Default.BiliResultResponsePgcFavoriteListResponse, parameters, cancellationToken).ConfigureAwait(false);
    ...
    public async Task<(IReadOnlyList<VideoFavoriteFolder> Folders, IReadOnlyList<string> ContainerIds)> GetPlayingVideoFavoriteFoldersAsync(string aid, CancellationToken cancellationToken)
    {
        ...
        var responseObj = await GetAsync(BiliApis.Account.FavoriteList, JsonContext.Default.BiliDataResponseFavoriteListResponse, parameters, cancellationToken).ConfigureAwait(false);
    ...
    public async Task<VideoFavoriteFolderDetail> GetVideoFavoriteFolderDetailAsync(VideoFavoriteFolder folder, int pageNumber, CancellationToken cancellationToken = default)
    {
        ...
        var responseObj = await GetAsync(BiliApis.Account.VideoFavoriteDelta, JsonContext.Default.BiliDataResponseVideoFavoriteListResponse, parameters, cancellationToken).ConfigureAwait(false);
    ...
    public async Task<(IReadOnlyList<VideoInformation> Videos, int TotalCount, int? NextPageNumber)> GetUgcSeasonVideosAsync(VideoFavoriteFolder folder, int pageNumber, CancellationToken cancellationToken)
    {
        ...
        var response = await _httpClient.SendAsync(request, cancellationToken).ConfigureAwait(false);
        var responseObj = await BiliHttpClient.ParseAsync(response, JsonContext.Default.BiliDataResponseUgcSeasonDetailResponse).ConfigureAwait(false);
    ...
    private async Task<T> GetAsync<T>(string url, JsonTypeInfo<T> typeInfo, Dictionary<string, string>? parameters = default, CancellationToken cancellationToken = default)
    {
        await _authenticationService.EnsureTokenAsync(cancellationToken).ConfigureAwait(false);
        var request = BiliHttpClient.CreateRequest(HttpMethod.Get, new Uri(url));
        _authenticator.AuthroizeRestRequest(request, parameters);
        var response = await _httpClient.SendAsync(request, cancellationToken).ConfigureAwait(false);
        return await BiliHttpClient.ParseAsync(response, typeInfo).ConfigureAwait(false);
    }
}
