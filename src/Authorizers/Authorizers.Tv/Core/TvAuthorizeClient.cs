...
internal sealed partial class TVAuthorizeClient
{
    ...
    public async Task<TVQRCode> GetQRCodeAsync(CancellationToken cancellationToken = default)
    {
        ...
        var responseObj = await BiliHttpClient.ParseAsync(response, JsonContext.Default.BiliDataResponseTVQRCode).ConfigureAwait(false);
    ...
    public async Task<BiliToken?> WaitQRCodeScanAsync(TVQRCode code, CancellationToken cancellationToken = default)
    {
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            catch (KernelException ke)
            {
                ...
                {
                    var error = JsonSerializer.Deserialize(ke.InnerException.Message, JsonContext.Default.BiliResponse);
                    if (error != null)
                    {
    ...
    public async Task SignInWithCookiesAsync(IDictionary<string, string> cookies, CancellationToken cancellationToken)
    {
        ...
        _basicAuthenticator.AuthroizeRestRequest(request, parameters, new BiliAuthorizeExecutionSettings() { ForceNoToken = true, NeedCSRF =  true, RequireCookie = true });
        var response = await _httpClient.SendAsync(request, cancellationToken).ConfigureAwait(false);
        _isScanCheckInvoking = false;
    ...
    private async Task<BiliToken> RefreshTokenAsync(CancellationToken cancellationToken = default)
    {
        ...
        var response = await _httpClient.SendAsync(request, cancellationToken).ConfigureAwait(false);
        var responseObj = await BiliHttpClient.ParseAsync(response, JsonContext.Default.BiliDataResponseBiliToken).ConfigureAwait(false);
