...
public interface IViewLaterService
{
    ...
    /// 获取稍后再看视频.
    /// </summary>
    Task<(IReadOnlyList<VideoInformation>? Videos, int Count, int NextPageNumber)> GetViewLaterSetAsync(int pageNumber = 0, CancellationToken cancellationToken = default);
