...
internal sealed class MyProfileModule : IFeatureModule
{
    ...
    private async Task DisplayUserInformationAsync()
    {
        ...
        table.AddRow(
            ...
            _myCommunityInfo.MomentCount.ToString(),
    ...
    private async Task DisplayFansAsync()
    {
        ...
        var (users, count, nextPage) = await profileService.GetMyFansAsync(1, _cancellationToken).ConfigureAwait(false);
        AnsiConsole.Clear();
    ...
    private async Task DisplayViewLaterAsync()
    {
        ...
        var (videos, count, nextPage) = await profileService.GetViewLaterSetAsync(1, _cancellationToken).ConfigureAwait(false);
        AnsiConsole.Clear();
